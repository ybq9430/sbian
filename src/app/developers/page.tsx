...
import { errorMessage, type ApiKeyData } from "@/types/api";

interface ApiKeyWithFullKey extends ApiKeyData {
  fullKey?: string;
}

interface WebhookWithDetails {
  id: string;
  url: string;
  events: string;
  active: boolean;
  secret?: string;
  deliveries?: { id: string; event: string; status: string; createdAt: string }[];
  createdAt: string;
}
...
export default function DeveloperPage() {
  ...
  const [keys, setKeys] = useState<ApiKeyWithFullKey[]>([]);
  const [webhooks, setWebhooks] = useState<WebhookWithDetails[]>([]);
  ...
  async function deleteKey(id: string) {
    try {
      const res = await fetch("/api/api-keys", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id }) });
      if (!res.ok) throw new Error(`Request failed: ${res.status}`);
    } catch (err: unknown) {
      setError(errorMessage(err));
      return;
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        <h1 className="text-2xl font-bold mb-2 dark:text-gray-100">Developer platform</h1>
        <p className="text-sm text-gray-500 mb-8 dark:text-gray-400">API keys, webhooks, and integrations for your products.</p>
        ...
        {tab === "keys" && (
          <div className="space-y-6">
            ...
            <div className="card overflow-x-auto">
              <table className="w-full text-sm"><thead><tr className="text-left text-gray-500 border-b dark:text-gray-400 dark:border-gray-700"><th className="pb-2">Name</th><th className="pb-2">Key</th><th className="pb-2">Scopes</th><th className="pb-2">Created</th><th className="pb-2" /></tr></thead>
                <tbody>{keys.map(k => (
                  <tr key={k.id} className="border-b border-gray-50 dark:border-gray-800"><td className="py-2 font-medium">{k.name}</td><td className="py-2 font-mono text-xs">{showFullKey[k.id] ? k.fullKey || k.key : k.key}</td><td className="py-2 text-xs">{k.scopes}</td><td className="py-2 text-xs text-gray-500 dark:text-gray-400">{new Date(k.createdAt).toLocaleDateString()}</td><td className="py-2"><button onClick={() => deleteKey(k.id)} className="text-xs text-red-500 hover:underline">Revoke</button></td></tr>
                ))}</tbody></table>
              {keys.length === 0 && <p className="text-sm text-gray-400 text-center py-4 dark:text-gray-500">No API keys yet.</p>}
            </div>
          </div>
        )}
            ...
            <div className="space-y-4">
              {webhooks.map(w => (
                <div key={w.id} className="card">
                  <div className="flex items-center justify-between mb-2"><span className="font-medium text-sm">{w.url}</span><span className="text-xs bg-green-100 text-green-700 px-2 py-0.5 rounded-full dark:bg-green-900/50 dark:text-green-300">{w.active ? "Active" : "Inactive"}</span></div>
                  <div className="text-xs text-gray-500 dark:text-gray-400 space-x-4"><span>Events: {w.events}</span><span>Secret: {w.secret?.slice(0, 12)}...</span><span>Deliveries: {w.deliveries?.length || 0}</span></div>
                  <div className="mt-2"><button onClick={() => deleteWebhook(w.id)} className="text-xs text-red-500 hover:underline">Remove</button></div>
                </div>
              ))}
              {webhooks.length === 0 && <p className="text-sm text-gray-400 text-center py-4 dark:text-gray-500">No webhooks configured.</p>}
            </div>
          </div>
        )}
          ...
          <div className="card space-y-6">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              {[
                ...
                <div key={i} className="p-3 bg-gray-50 rounded-lg dark:bg-gray-700">
                  <span className="text-xs font-mono bg-gray-200 px-1.5 py-0.5 rounded mr-2 dark:bg-gray-600 dark:text-gray-300">{ep.method}</span>
                  <span className="text-sm font-mono">{ep.path}</span>
                  <p className="text-xs text-gray-500 mt-1 dark:text-gray-400">{ep.desc}</p>
                </div>
              ))}
            </div>
            <div>
              <h3 className="font-semibold mb-2">Rate limits</h3>
              <p className="text-sm text-gray-500 dark:text-gray-400">Free: 100 req/min. Pro: 1000 req/min. Enterprise: custom.</p>
            </div>
          </div>
        )}
      </div>
