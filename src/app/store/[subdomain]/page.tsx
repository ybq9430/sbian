...
export default function PublicStorePage() {
  ...
  const [data, setData] = useState<{ subdomain: string; title: string; description: string; theme: string; logoUrl: string; products: ProductData[]; error?: string } | null>(null);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    fetch(`/api/storefront?subdomain=${subdomain}`)
      .then(r => { if (!r.ok) throw new Error(`Request failed: ${r.status}`); return r.json(); })
      .then(setData)
      .catch(err => setError(errorMessage(err)));
  }, [subdomain]);

  if (error) return <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex items-center justify-center"><div className="text-red-500 dark:text-red-400 p-4 text-center bg-red-50 dark:bg-red-900/30 rounded-lg">{error}</div></div>;
  if (!data) return <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex items-center justify-center">Loading...</div>;
  if (data.error) return <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex items-center justify-center"><div className="text-center"><h1 className="text-2xl font-bold mb-2">Store not found</h1><Link href="/" className="text-brand-600">Go home</Link></div></div>;
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {data.products?.map((p: ProductData) => (
