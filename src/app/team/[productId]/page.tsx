...
export default function TeamPage() {
  ...
  useEffect(() => {
    ...
    if (status === "authenticated") {
      ...
      fetch(`/api/team?productId=${productId}`).then(r => r.json()).then(setMembers);
    }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-3xl mx-auto px-4 py-8">
        <div className="mb-8">
          <h1 className="text-2xl font-bold">Team: {product?.title}</h1>
          <p className="text-sm text-gray-500">Collaborate with others on this product</p>
        </div>
          ...
          <div className="space-y-3">
            ...
            {members.map(m => (
              <div key={m.id} className="flex items-center gap-3">
                ...
                <div className="flex-1"><p className="text-sm font-medium">{m.user?.name}</p><p className="text-xs text-gray-400">{m.user?.email}</p></div>
                <span className="text-xs bg-gray-100 px-2 py-0.5 rounded-full capitalize">{m.role}</span>
                <button onClick={() => removeMember(m.userId)} className="text-xs text-red-500 hover:underline">Remove</button>
              </div>
            ))}
            {members.length === 0 && <p className="text-sm text-gray-400 text-center py-4">No team members yet. Invite collaborators above.</p>}
          </div>
        </div>
      </div>
    </div>
