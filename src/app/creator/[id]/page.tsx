...
export default function CreatorProfilePage() {
  ...
  const [followData, setFollowData] = useState<{ followerCount: number; followingCount: number } | null>(null);
  ...
  useEffect(() => {
    fetch(`/api/follow?userId=${id}`)
      ...
      .then(d => { setFollowData(d); })
      .catch(err => setError(errorMessage(err)));
    fetch(`/api/products`)
      .then(r => { if (!r.ok) throw new Error(`Request failed: ${r.status}`); return r.json(); })
      .then(d => {
        const userProducts = d.products.filter((p: ProductData) => p.sellerId === id);
  ...
  async function handleFollow() {
    ...
    try {
      ...
      setFollowData((prev) => ({ followerCount: (prev?.followerCount ?? 0) + (data.following ? 1 : -1), followingCount: prev?.followingCount ?? 0 }));
    } catch (err: unknown) {
      setError(errorMessage(err));
    }
  }

  if (error) return <div className="min-h-screen bg-gray-50 dark:bg-gray-900"><Navbar /><div className="text-red-500 dark:text-red-400 p-4 text-center bg-red-50 dark:bg-red-900/30 rounded-lg mb-4 max-w-4xl mx-auto mt-8">{error}</div></div>;
  if (!profile) return <div className="min-h-screen bg-gray-50 dark:bg-gray-900"><Navbar /><div className="max-w-4xl mx-auto p-8">Loading...</div></div>;
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
          ...
          <div className="flex items-start gap-6">
            ...
            <div className="flex-1">
              <div className="flex items-center justify-between">
                <div>
                  <h1 className="text-2xl font-bold dark:text-gray-100">{profile.name}</h1>
                  <p className="text-sm text-gray-500 dark:text-gray-400">@{profile.name?.toLowerCase().replace(/\s+/g, "")}</p>
                </div>
                {user && user?.id !== id && (
        ...
        <h2 className="text-xl font-bold mb-6 dark:text-gray-100">Products</h2>
