...
export default function CommunityPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/discussions?productId=${productId}`).then(r => r.json()).then(setDiscussions);
  }, [productId]);
  ...
  async function handleReply(discussionId: string, parentId?: string) {
    ...
    if (res.ok) {
      const updated = await fetch(`/api/discussions?productId=${productId}`).then(r => r.json());
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-2xl font-bold">{product?.title || "Community"}</h1>
            <p className="text-sm text-gray-500">Discussions & Q&A</p>
          </div>
              ...
              <div key={d.id} className="card">
                <div className="flex items-start gap-3 mb-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-gray-400">{d.user?.name} · {formatDate(d.createdAt)}</p>
                  </div>
                </div>
                <p className="text-sm text-gray-600 mb-4">{d.body}</p>

                {/* Replies */}
                {d.replies?.length > 0 && (
                  <div className="border-t border-gray-100 pt-3 space-y-3 ml-6">
                    {d.replies.map((r: any) => (
                      <div key={r.id}>
                        <div className="flex items-start gap-2">
                          ...
                          <div className="flex-1">
                            <div className="flex items-center gap-2"><span className="text-xs font-medium">{r.user?.name}</span><span className="text-[10px] text-gray-400">{formatDate(r.createdAt)}</span></div>
                            ...
                            {r.children?.map((c: any) => (
                              <div key={c.id} className="flex items-start gap-2 mt-2 ml-4">
                                ...
                                <div><span className="text-[11px] font-medium">{c.user?.name}</span><p className="text-xs text-gray-500">{c.body}</p></div>
                              </div>
                            ))}
                          </div>
                        </div>
