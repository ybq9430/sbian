...
interface ABTestWithProduct {
  ...
  variants: { id: string; name: string; title?: string | null; description?: string | null; price?: number | null; impressions: number; conversions: number }[];
  createdAt: string;
}
...
export default function ABTestsPage() {
  ...
  const [tests, setTests] = useState<ABTestWithProduct[]>([]);
  ...
  async function createTest(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.ok) { setTests([t, ...tests]); setShowCreate(false); setMessage("Test created! Start it to begin collecting data."); }
    } catch (err: unknown) {
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-8">
          <div><h1 className="text-2xl font-bold dark:text-gray-100">A/B Tests</h1><p className="text-sm text-gray-500 dark:text-gray-400">Test different titles, descriptions, and prices to optimize conversions.</p></div>
        ...
        {showCreate && (
          <form onSubmit={createTest} className="card mb-8 space-y-4">
            ...
            <div><label className="block text-sm font-medium mb-1">Traffic split (%)</label><input type="range" min="10" max="90" value={form.traffic * 100} onChange={e => setForm({ ...form, traffic: parseInt(e.target.value) / 100 })} className="w-full" /><p className="text-xs text-gray-400 dark:text-gray-500">{Math.round(form.traffic * 100)}% see variant · {Math.round((1 - form.traffic) * 100)}% see original</p></div>
            <div className="grid grid-cols-2 gap-4">
              {form.variants.map((v, i) => (
                <div key={i} className="p-4 bg-gray-50 rounded-lg dark:bg-gray-700">
        ...
        <div className="space-y-4">
          {tests.map(t => (
            <div key={t.id} className="card">
              <div className="flex items-center justify-between mb-4">
                <div><h3 className="font-semibold">{t.name}</h3><p className="text-xs text-gray-500 dark:text-gray-400">{t.product?.title} · Traffic: {Math.round(t.traffic * 100)}%</p></div>
                <span className={`text-xs px-2 py-0.5 rounded-full ${t.status === "active" ? "bg-green-100 text-green-700 dark:bg-green-900/50 dark:text-green-300" : t.status === "draft" ? "bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-400" : "bg-blue-100 text-blue-700 dark:bg-blue-900/50 dark:text-blue-300"}`}>{t.status}</span>
              </div>
              <div className="grid grid-cols-2 gap-4">
                {t.variants.map((v: ABTestWithProduct["variants"][number]) => (
                  <div key={v.id} className="p-3 bg-gray-50 rounded-lg dark:bg-gray-700">
                    <div className="flex items-center justify-between mb-2"><span className="text-sm font-medium">{v.name}</span><span className="text-xs text-gray-400 dark:text-gray-500">{v.impressions} views</span></div>
                    <p className="text-xs text-gray-600 truncate dark:text-gray-400">{v.title || t.product?.title}</p>
          ...
          {tests.length === 0 && !showCreate && <div className="card text-center py-12"><p className="text-gray-400 dark:text-gray-500">No A/B tests yet. Create one to optimize your product pages.</p></div>}
        </div>
      </div>
    </div>
  );
