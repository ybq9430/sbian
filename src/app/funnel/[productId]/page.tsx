...
export default function ConversionFunnelPage() {
  ...
  useEffect(() => {
    ...
    if (status === "authenticated") {
      fetch(`/api/analytics/conversion?productId=${productId}`).then(r => r.json()).then(setData);
      fetch(`/api/products/${productId}`).then(r => r.json()).then(setProduct);
    }
  }, [productId, status, router]);

  if (!data || !product) return <div className="min-h-screen bg-gray-50"><Navbar /><div className="max-w-5xl mx-auto p-8">Loading...</div></div>;

  return (
    <div className="min-h-screen bg-gray-50">
      <Navbar />
      <div className="max-w-5xl mx-auto px-4 py-8">
        <h1 className="text-2xl font-bold mb-2">Conversion funnel: {product.title}</h1>
        <p className="text-sm text-gray-500 mb-8">Track how visitors convert into buyers</p>
          ...
          <div className="card">
            ...
            {Object.keys(data.sources).length > 0 ? (
              <div className="space-y-3">
                {Object.entries(data.sources).map(([source, count]: [string, any]) => (
              ...
              <p className="text-sm text-gray-400">No traffic data yet. Share your product to start tracking.</p>
            )}
          </div>
        </div>
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            {[
              ...
              <div key={i} className="p-4 bg-gray-50 rounded-lg flex gap-3">
                ...
                <div><h4 className="text-sm font-semibold mb-1">{tip.title}</h4><p className="text-xs text-gray-500">{tip.desc}</p></div>
              </div>
            ))}
          </div>
        </div>
